...
def predict_efficiency():
    ...
    if not (VELOCITY_MIN_KMH <= velocity_kmh <= VELOCITY_MAX_KMH):
        return jsonify({"error": f"velocity_kmh must be between {VELOCITY_MIN_KMH} and {VELOCITY_MAX_KMH} km/h"}), 400
